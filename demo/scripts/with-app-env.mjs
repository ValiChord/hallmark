...
function main(argv) {
  ...
  // Windows: `vite` is an extensionless shim and Node cannot resolve it without a
  // shell; since Node 20 it also refuses to spawn `vite.cmd` directly (EINVAL,
  // the CVE-2024-27980 mitigation). Without this the dev server cannot start on
  // Windows at all. POSIX keeps the direct spawn.
